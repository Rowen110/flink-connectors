...
public class ClickHouseRowConverter implements Serializable {
    ...
    public ClickHouseRowConverter(RowType rowType, boolean collapsing, boolean replacing) {
    ...
    public ClickHousePreparedStatement toClickHouse(
            RowData rowData, ClickHousePreparedStatement statement) throws SQLException {
        ...
        if (this.replacing) {
            switch (rowData.getRowKind()) {
                case INSERT:
                case UPDATE_AFTER:
                    statement.setInt(rowData.getArity() + 1, 0);
                    break;
                case UPDATE_BEFORE:
                case DELETE:
                    statement.setInt(rowData.getArity() + 1, 1);
    ...
    protected DeserializationConverter createToFlinkConverter(LogicalType type) {
        ...
        switch (type.getTypeRoot()) {
            ...
            case TIME_WITHOUT_TIME_ZONE:
                return val ->
                        Integer.valueOf(
                                (int) (((Time) val).toLocalTime().toNanoOfDay() / 1000000L));
    ...
    protected SerializationConverter wrapIntoNullableToClickHouseConverter(
            SerializationConverter converter, LogicalType type) {
        int sqlType =
        ...
        switch (type.getTypeRoot()) {
            ...
            case TIME_WITHOUT_TIME_ZONE:
                return (val, index, statement) ->
                        statement.setTime(
                                index + 1,
                                Time.valueOf(LocalTime.ofNanoOfDay(val.getInt(index) * 1000000L)));
            case TIMESTAMP_WITH_TIME_ZONE:
            case TIMESTAMP_WITHOUT_TIME_ZONE:
    ...
    interface SerializationConverter extends Serializable {
        void serialize(
                RowData param1RowData, int param1Int, PreparedStatement param1PreparedStatement)
                throws SQLException;
    }
}
