...
public class ClickHouseDynamicTableFactory implements DynamicTableSinkFactory {
  ...
  public static final ConfigOption<String> CLUSTER_NAME = ConfigOptions
      ...
      .withDescription("the ClickHouse cluster name. Default to `default_cluster`.");
  ...
  public static final ConfigOption<String> TABLE_COLLAPSING_FIELD = ConfigOptions
      .key("table.collapsing.field")
      .stringType()
      .noDefaultValue()
      .withDescription("Name of the column of the CollapsingMergeTree table");
  ...
  public static final ConfigOption<Duration> SINK_FLUSH_INTERVAL = ConfigOptions
  ...
  public static final ConfigOption<Boolean> SINK_WRITE_LOCAL = ConfigOptions
      .key("sink.write-local")
      .booleanType()
      .defaultValue(false)
      .withDescription("directly write to local tables in case of Distributed table.");

  public static final ConfigOption<String> SINK_WRITE_LOCAL_NODES = ConfigOptions
      .key("sink.write-local-nodes")
      .stringType()
      .noDefaultValue()
      .withDescription(
          "nodes in Distributed table, if not set, use 'SELECT shard_num, host_address, port FROM system.clusters WHERE cluster = ? and replica_num = 1;' to get nodes");

  public static final ConfigOption<String> SINK_PARTITION_STRATEGY = ConfigOptions
      .key("sink.partition-strategy")
      .stringType()
      .defaultValue("balanced")
      .withDescription("partition strategy. available: balanced, hash, shuffle.");

  public static final ConfigOption<String> SINK_PARTITION_KEY = ConfigOptions
      .key("sink.partition-key")
      .stringType()
      .noDefaultValue()
      .withDescription("partition key used for hash strategy. separated by ','");
  ...
  private void validateConfigOptions(ReadableConfig config) {
    ...
    if (!Arrays.asList(ClickHousePartitioner.HASH, ClickHousePartitioner.BALANCED, ClickHousePartitioner.SHUFFLE)
        .contains(partitionStrategy)) {
      throw new IllegalArgumentException("Unknown sink.partition-strategy `" + partitionStrategy + "`");
    } else if (partitionStrategy.equals(ClickHousePartitioner.HASH) &&
  ...
  private ClickHouseOptions getOptions(ReadableConfig config) {
    return (new ClickHouseOptions.Builder()).withUrl(config.get(URL)).withUsername(config.get(USERNAME))
        .withPassword(config.get(PASSWORD)).withClusterName(config.get(CLUSTER_NAME))
        .withDatabaseName(config.get(DATABASE_NAME)).withTableName(config.get(TABLE_NAME))
        .withTableCollapsingField(config.get(TABLE_COLLAPSING_FIELD)).withBatchSize(config.get(SINK_BATCH_SIZE))
        .withFlushInterval(config.get(SINK_FLUSH_INTERVAL)).withMaxRetries(config.get(SINK_MAX_RETRIES))
        .withWriteLocal(config.get(SINK_WRITE_LOCAL)).withWriteLocalNodes(config.get(SINK_WRITE_LOCAL_NODES))
        .withPartitionStrategy(config.get(SINK_PARTITION_STRATEGY)).withPartitionKey(config.get(SINK_PARTITION_KEY))
        .withIgnoreDelete(config.get(SINK_IGNORE_DELETE)).build();
  }
}
