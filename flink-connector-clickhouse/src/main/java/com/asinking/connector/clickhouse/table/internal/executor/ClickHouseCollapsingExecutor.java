...
public class ClickHouseCollapsingExecutor implements ClickHouseExecutor {
  private static final long serialVersionUID = 1L;

  private static final Logger LOG = LoggerFactory.getLogger(ClickHouseCollapsingExecutor.class);
  ...
  public ClickHouseCollapsingExecutor(String sql, ClickHouseRowConverter converter, ClickHouseOptions options) {
    this.sql = sql;
  ...
  private class ExecuteBatchService extends AbstractExecutionThreadService {
    ...
    protected void run() throws Exception {
      while (isRunning()) {
        synchronized (ClickHouseCollapsingExecutor.this) {
          ClickHouseCollapsingExecutor.this.wait(ClickHouseCollapsingExecutor.this
              .flushInterval.toMillis());
          if (!ClickHouseCollapsingExecutor.this.batch.isEmpty()) {
            ClickHouseCollapsingExecutor.this.stmt.clearBatch();
            for (RowData record : ClickHouseCollapsingExecutor.this.batch) {
              ClickHouseCollapsingExecutor.this.converter.toClickHouse(record, ClickHouseCollapsingExecutor.this.stmt);
              ClickHouseCollapsingExecutor.this.stmt.addBatch();
            }
            attemptExecuteBatch(ClickHouseCollapsingExecutor.this.stmt, ClickHouseCollapsingExecutor.this.batch);
          }
        }
      }
    }
    ...
    private void attemptExecuteBatch(ClickHousePreparedStatement stmt, List<RowData> batch) throws IOException {
      for (int i = 1; i <= ClickHouseCollapsingExecutor.this.maxRetries; i++) {
        try {
          stmt.executeBatch();
          batch.clear();
          break;
        } catch (SQLException e) {
          ClickHouseCollapsingExecutor.LOG
              .error("ClickHouse executeBatch error, retry times = {}", Integer.valueOf(i), e);
          if (i >= ClickHouseCollapsingExecutor.this.maxRetries) {
