  ...
  public static class Builder implements Serializable {
    ...
    public AbstractClickHouseOutputFormat build() {
      ...
      if (this.hasPrimaryKey) {
        ...
        this.options.getTableCollapsingField().ifPresent(field -> {
          String[] newField = new String[this.fieldNames.length + 1];
          System.arraycopy(this.fieldNames, 0, newField, 0, this.fieldNames.length);
    ...
    private AbstractClickHouseOutputFormat.ExecutorFactory createExecutorFactory(ClickHouseRowConverter converter) {
      if (this.hasPrimaryKey) {
        if (this.options.getTableCollapsingField().isPresent()) {
          LOG.info("use collapsing");
          return () -> ClickHouseExecutor
              .createCollapsingExecutor(this.options.getTableName(), this.fieldNames, converter, this.options);
        }
        throw new IllegalArgumentException("upsert mode. need to set table.collapsing.field`");
      }
      LOG.info("use insert only");
      String sql = ClickHouseStatementFactory.getInsertIntoStatement(this.options
          .getTableName(), this.fieldNames);
      return () -> new ClickHouseBatchExecutor(sql, converter, this.options.getFlushInterval(),
          this.options.getMaxRetries());
    }

    private ClickHouseBatchOutputFormat createBatchOutputFormat(ClickHouseRowConverter converter) {
      return new ClickHouseBatchOutputFormat(new ClickHouseConnectionProvider(this.options),
          createExecutorFactory(converter), this.options);
    }
    ...
    private ClickHouseShardOutputFormat createShardOutputFormat(LogicalType[] logicalTypes,
      ...
      switch (this.options.getPartitionStrategy()) {
        ...
        case ClickHousePartitioner.HASH:
          ...
          for (String partitionKey : this.options.getPartitionKey().split(",")) {
            ...
            if (index == -1) {
              throw new IllegalArgumentException("Partition key `" + partitionKey
                  .trim() + "` not found in table schema");
            }
      ...
      return new ClickHouseShardOutputFormat(new ClickHouseConnectionProvider(this.options), partitioner,
          createExecutorFactory(converter), this.options);
    }
  }
}
